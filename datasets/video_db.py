...
class VideoDataset(data.Dataset):
    def __init__(self,
        ...
        self.print_pil_num = print_pil_num
        self.num_samples = 0
        ...
        self.mode = mode
        self.sample_grad_cam = sample_grad_cam
        self.sample_grad_cam_audio = sample_grad_cam_audio
    ...
    def __getitem__(self, index):
        if self.mode == 'clip':
            try:
                ...
                sample = self._get_clip(sample_idx, video_ctr, audio_ctr, v_ss, a_ss, video_clip_duration=v_dur,
            ...
            try:
                ...
                if vs is None:
                    ...
                    start_time = vs
                    final_time = vf
                if self.return_audio and vs is None:
                    start_time = max(0, ss) if ss < 0 else ss
                    final_time = sf
                ...
                sample = self._get_clip(index, video_ctr, audio_ctr, start_time, start_time,
                ...
                if self.return_video:
                    nf = sample['frames0'].shape[1]
                    chunk_size = int(self.video_clip_duration * self.video_fps)
                    if chunk_size >= nf:
                        chunks['frames0'] = torch.stack([sample['frames0'] for _ in range(self.clips_per_video)])
                    else:
                        timestamps = np.linspace(0, max(nf - chunk_size, 1), self.clips_per_video).astype(int)
                        chunks['frames0'] = torch.stack([sample['frames0'][:, ss:ss + chunk_size] for ss in timestamps])

                if self.return_audio:
                    nf = sample['audio0'].shape[1]
                    chunk_size = int(self.audio_clip_duration * self.audio_fps_out)
                    if chunk_size >= nf:
                        chunks['audio0'] = torch.stack([sample['audio0'] for _ in range(self.clips_per_video)])
                    else:
                        timestamps = np.linspace(0, max(nf - chunk_size, 1), self.clips_per_video).astype(int)
                        chunks['audio0'] = torch.stack([sample['audio0'][:, ss:ss + chunk_size] for ss in timestamps])

                if self.return_labels:
                    chunks['label'] = sample['label']

                if self.return_index:
                    ts = torch.from_numpy(np.linspace(start_time, final_time - self.video_clip_duration,
    ...
    def _get_time_lims(self, video_ctr, audio_ctr):
        ...
        if audio_ctr is not None:
            ...
            audio_st = audio_stream.start_time * tbase if audio_stream.start_time is not None else 0.
    ...
    def _sample_snippet(self, video_ctr, audio_ctr):
        ...
        if not self.return_video:
            ...
            if self.audio_clip_duration > audio_duration:
                ...
                duration = random.uniform(min_d, max_d)
                sample_ss_v = random.uniform(audio_st, audio_ft - duration)
                return sample_ss_v, duration, sample_ss_v, duration
        else:  # both need to return
    ...
    def _get_audio_sample(self, samples, audio_clip_duration, audio_fps):
        t = int(audio_clip_duration * audio_fps)
        samples_pad_ = None
        if t > samples.shape[1]:
            samples_pad_ = np.pad(samples, ((0, 0), (0, t - samples.shape[1])), 'constant', constant_values=0)
        else:
            samples_pad_ = samples
        return samples_pad_

    def _get_clip(self, clip_idx, video_ctr, audio_ctr, video_start_time, audio_start_time, video_clip_duration=None,
        ...
        if self.return_video:
            ...
            # if self.print_pil_num < 1:
            #     self.print_pil_num += 1
            #     # print(frames[0])
            #     frames[0].show()
            # try:
            #     frames_tensor = []
            #     for ii in range(len(frames)):
            #         frame_tensor = tfs.ToTensor()(frames[ii])
            #         frames_tensor.append(frame_tensor)
            #     frames_all = torch.stack(frames_tensor, dim=1)
            #     # print(frames_all.size())
            #     sample['frames{}'.format(2)] = frames_all
            # except Exception:
            #     sample['frames{}'.format(2)] = None
            # if self.sample_grad_cam:
            # frames_tensor = defaultdict(torch.Tensor)
            # for ii in range(len(frames)):
            #     frame_tensor = tfs.ToTensor()(frames[ii])
            #     frames_tensor.update({ii: frame_tensor})
            # sample['frames{}'.format(2)] = frames_tensor
            if self.sample_grad_cam:
                frames_tensor = []
                num_frames = int(self.video_clip_duration * self.video_fps)
                for ii in range(len(frames)):
                    frames_crop = tfs.Resize((224, 224))(frames[ii])
                    frame_tensor = tfs.ToTensor()(frames_crop)
                    frames_tensor.append(frame_tensor)
                while True:
                    n_missing = num_frames - len(frames_tensor)
                    if n_missing > 0:
                        if len(frames_tensor) >= n_missing:
                            frames_tensor = frames_tensor + frames_tensor[ :n_missing]
                        else:
                            frames_tensor = frames_tensor + frames_tensor[ :len(frames_tensor)]
                    else:
                        break
                frames_all = torch.stack(frames_tensor, dim=1)
                # print(frames[0])
                # print(frames_all.size())
                sample['frames{}'.format(2)] = frames_all
            for clip_video_i in range(2):
                frames_ones = frames
                # print(frames_ones.shape)
                if self.video_transform is not None:
                    for t in self.video_transform:
                        frames_ones = t(frames_ones)
                # print(frames_ones.shape)
                sample['frames{}'.format(clip_video_i)] = frames_ones
            # sample['frames{}'.format(2)] = frames_all

            audio_start_time = audio_start_time - (video_start_time - start_time)

        if self.return_audio:
            samples, rate = av_wrappers.av_load_audio(
                audio_ctr,
            ...
            # print(samples.shape)
            if self.sample_grad_cam_audio:
                samples_pad = self._get_audio_sample(samples, audio_clip_duration, self.audio_fps)
                sample['audio{}'.format(2)] = samples_pad
                # print(type(samples))  # <class 'numpy.ndarray'>  # (1, 48000)
                # print(samples.dtype)  # float64
            for clip_audio_i in range(2):
                samples_ones = samples
                rate_ones = rate
                if self.audio_transform is not None:
                    if isinstance(self.audio_transform, list):
                        for t in self.audio_transform:
                            samples_ones, rate_ones = t(samples_ones, rate_ones, audio_clip_duration)
                    else:
                        samples_ones, rate_ones = self.audio_transform(samples_ones, rate_ones)
                sample['audio{}'.format(clip_audio_i)] = samples_ones
