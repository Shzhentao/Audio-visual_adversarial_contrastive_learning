...
class AV_Wrapper(nn.Module):
    def __init__(self, video_models, audio_models, proj_dim=128, use_shuffle=False):
        super(AV_Wrapper, self).__init__()
        self.use_shuffle = use_shuffle
        self.video_model_q = video_models[0]
        self.video_model_k = video_models[1]
        
        self.audio_model_q = audio_models[0]
        self.audio_model_k = audio_models[1]
        ...
        if proj_dim is not None:
            ...
            self.audio_proj_q = Head(audio_models[0].out_dim, proj_dim)
            self.audio_proj_k = Head(audio_models[0].out_dim, proj_dim)

            for param_q, param_k in zip(self.video_proj_q.parameters(), self.video_proj_k.parameters()):
                param_k.data.copy_(param_q.data)  # initialize
                param_k.requires_grad = False  # not update by gradient

            for param_q, param_k in zip(self.audio_proj_q.parameters(), self.audio_proj_k.parameters()):
                param_k.data.copy_(param_q.data)  # initialize
                param_k.requires_grad = False  # not update by gradient
...
def av_wrapper(video_backbone, video_backbone_args, audio_backbone, audio_backbone_args, proj_dim=128, checkpoint=None, use_shuffle=False):
    ...
    audio_model_q = models.__dict__[audio_backbone](**audio_backbone_args)
    audio_model_k = models.__dict__[audio_backbone](**audio_backbone_args)
    audio_models = [audio_model_q, audio_model_k]
    model = AV_Wrapper(video_models, audio_models, proj_dim=proj_dim, use_shuffle=use_shuffle)
