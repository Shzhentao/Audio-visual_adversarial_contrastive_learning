...
def prep_environment(args, cfg):
    ...
    num_file = 0
    while os.path.exists('{}/train{}.log'.format(model_dir, num_file)):
        num_file += 1
    log_fn = '{}/train{}.log'.format(model_dir, num_file)
    ...
    if cfg['log2tb'] and args.rank == 0:
        num_tensorboard = 0
        while os.path.exists('{}/tensorboard{}'.format(model_dir, num_tensorboard)):
            num_tensorboard += 1
        tb_dir = '{}/tensorboard{}'.format(model_dir, num_tensorboard)
...
def build_avac_criterion(cfg, logger=None):
    import models
    criterion = models.__dict__[cfg['name']](**cfg['args'])
    if logger is not None:
        # it will turn in there
        logger.add_line("=" * 30 + "   cross Model   " + "=" * 30)
        logger.add_line(str(criterion))
        logger.add_line("=" * 30 + "   cross Parameters   " + "=" * 30)
...
def build_optimizer(params_base_model, params_avac_criterion, cfg, logger=None):
    if cfg['name'] == 'sgd':
        optimizer_base_model = torch.optim.SGD(
            params=params_base_model,
            lr=cfg['lr']['base_lr'],
            momentum=cfg['momentum'],
            weight_decay=cfg['weight_decay'],
            # nesterov=cfg['nesterov']
        )
        optimizer_neg = torch.optim.SGD(
            params=params_avac_criterion,
            lr=cfg['lr']['negative_lr'],
            momentum=cfg['momentum'],
            weight_decay=cfg['negative_weight_decay'],
            # nesterov=cfg['nesterov']
        )

    elif cfg['name'] == 'adam':
        optimizer_base_model = torch.optim.Adam(
            params=params_base_model,
            lr=cfg['lr']['base_lr'],
            weight_decay=cfg['weight_decay'],
    ...
    # scheduler_base_model = torch.optim.lr_scheduler.MultiStepLR(optimizer_base_model,
    #                                                             milestones=cfg['lr']['milestones'],
    #                                                             gamma=cfg['lr']['gamma'])
    # scheduler_neg = torch.optim.lr_scheduler.MultiStepLR(optimizer_neg, milestones=cfg['lr']['milestones'],
    #                                                      gamma=cfg['lr']['gamma'])
    # scheduler_base_model = 1
    # scheduler_neg = 1
    optimizers = [optimizer_base_model, optimizer_neg]
    # schedulers = [scheduler_base_model, scheduler_neg]
    return optimizers
...
class CheckpointManager(object):
    ...
    def save(self, epoch, filename=None, eval_metric=0., **kwargs):
        ...
        if filename is None:
            ...
            save_checkpoint(state=state, is_best=False,model_dir=self.checkpoint_dir, filename='{}/{}'.format(self.checkpoint_dir, filename))
...
def save_checkpoint(state, is_best, model_dir='.', filename=None, num_file = 0):
    if filename is None:
        filename = '{}/checkpoint.pth.tar'.format(model_dir)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, '{}/model_best{}.pth.tar'.format(model_dir, num_file))
